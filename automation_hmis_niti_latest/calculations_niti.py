...
def fetch_data(url, indicator_mapping):
    try:
        ...
        f_path = os.path.join(PATH,"error_url_niti.csv")
...
def fetch_district_data(dates, year_dates, base_url, district_ids, area='district'):
    ...
    df_district.to_csv(os.path.join(PATH,sheet_name), index=False, encoding='utf-8')
    return {}
...
def write_df(new_df, file_name):
    ...
    if os.path.exists(f_path):
        ...
        old_df = pd.read_csv(f_path, encoding='utf-8')
