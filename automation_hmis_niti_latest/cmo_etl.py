"""Main etl file."""
import logging
import calculations_cmo as calc
import gramex.cache
import pandas as pd
import urllib3
import sys
...
print("length:districts",len(organisation_unit['uid_district'].unique()))
print("length:blocks",len(organisation_unit['uid_block'].unique()))
...
def calculate_scores(dates,year_dates,area='district'):
    ...
    if area == 'district':
        ...
        output_sheet = 'data_cmo/district_cmo_scores.csv'

    elif area == 'block':
        sheet_name = 'data_cmo/subindicator_scores_blocks_cmo.csv'
        map = 'indicator_mappings_block'
        output_sheet = 'data_cmo/block_cmo_scores.csv'
    ...
    for date in dates:
        df_district_scores = df_district_scores.append(
            calc.indicator_month(str(date), data, map))
    ...
    df_district_scores['perc_point'].replace(np.nan, 0,inplace=True)
...
def calculate_scores_2(date,year_dates,area='district', period='year'):
    ...
    if area == 'district':
        sheet_name = 'data_cmo/subindicator_scores_districts_cmo.csv'
        map = 'indicator_mappings'
        output_sheet = 'data_cmo/district_cmo_scores_'+period+'.csv'
        ...
        sheet_name = 'data_cmo/subindicator_scores_blocks_cmo.csv'
        map = 'indicator_mappings_block'
        output_sheet = 'data_cmo/block_cmo_scores_'+period+'.csv'
    ...
    if period == 'quarter':
        ...
        for date in dates:
            for _id in indicator_idss:
                if _id not in ["14", "15", "17", "20", "23", "21"]:
                    df_district_scores = df_district_scores.append(
                        calc.indicator_qa(date, data, map, 'indicator_'+_id, sub_period_df))
                # pdb.set_trace()
                if _id in ["15", "21"]:
                    df_district_scores = df_district_scores.append(
                        calc.calculate15_21_quarterly(date, data, map, 'indicator_'+_id, sub_period_df))
                if _id in ["14", "17", "20", "23"]:
                    df_district_scores = df_district_scores.append(
                        calc.calculate_14_17_20_23(date, data, map, 'indicator_'+_id, sub_period_df))
        ...
        print(year_lists)
        year_q.append(year_lists)
        print(year_q)



        # dates = config['year_ind_15']
        # breakpoint()
        for date in year_q:
            for _id in indicator_idss:
                if _id in ['15','21']:
    ...
    # for _id in indicator_idss:
    #     df_district_scores = df_district_scores.append(
    #         calc.calculate_14_15_17_20_21_23_quaterly(date, data, map, 'indicator_'+_id, sub_period_df))
...
"""
Commands
python cmo_etl.py fetch fetch_districts - Fetch district sub indicators
"""
# if len(sys.argv) > 1:
...
# # Blocks calculate
# elif operation == 'cal_div':
#     calculate_scores('division')
#     print('*******Success: Block score: Month')

# elif operation == "cal_qa_div":
#     calculate_scores_2('division', 'quarter')
#     print('*******Success: Block score: Quarter')

# elif operation == "cal_yr_div":
#     calculate_scores_2('division', 'year')
#     print('*******Success: Block score: Year')

def calculate_cmo_scores(start_date,end_date,get_year_date):
    d = end_date[0]
    print(int(d)%100)
    if (int(d)%100) == 1 or (int(d)%100) == 2 or (int(d)%100) == 3:
        g_year = (int(d)//100)-1
