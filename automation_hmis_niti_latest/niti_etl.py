...
# Read config_niti.yaml
config = gramex.cache.open('config_niti.yaml', 'yaml', rel=True)

# Dates


# Read dependency files
indicators_df = gramex.cache.open('data_niti/indicator_id_mapping_niti.csv',
                                  'csv', rel=True, encoding='utf-8')
indicators_df_block = gramex.cache.open(
    'data_niti/indicator_id_mapping_block_niti.csv', 'csv', rel=True, encoding='utf-8')
sub_indicator_df = gramex.cache.open(
    'data_niti/sub_indicator_data_niti.csv', 'csv', rel=True, nrows=60, encoding='utf-8')
sub_indicator_df_block = gramex.cache.open(
    'data_niti/sub_indicator_data_block_niti.csv', 'csv', rel=True, nrows=60, encoding='utf-8')
...
def calculate_scores(dates,year_dates,area='district'):
    ...
    if area == 'district':
        ...
        output_sheet = 'data_niti/district_niti_scores.csv'

    elif area == 'block':
        sheet_name = 'data_niti/subindicator_scores_blocks_niti.csv'
        map = 'indicator_mappings_block'
        output_sheet = 'data_niti/block_niti_scores.csv'
...
def calculate_scores_2(date,year_dates,area='district', period='year'):
    ...
    if area == 'district':
        sheet_name = 'data_niti/subindicator_scores_districts_niti.csv'
        map = 'indicator_mappings'
        output_sheet = 'data_niti/district_niti_scores_'+period+'.csv'
        ...
        sheet_name = 'data_niti/subindicator_scores_blocks_niti.csv'
        map = 'indicator_mappings_block'
        output_sheet = 'data_niti/block_niti_scores_'+period+'.csv'
...
"""
Commands
python niti_etl.py fetch fetch_districts - Fetch district sub indicators
"""
# if len(sys.argv) > 1:
...
def calculate_niti_scores(start_date,end_date,get_year_date):
    ...
    # get_year_date = [str(get_year_date)]
    # # Get date ranges
