/* exported gauage */
/* global url */
function gauage(selector, configuration) {
  ...
  var svg = d3
    ...
    .attr("viewBox", "5 0 " + config.width + " " + config.height);
  var bla = svg
    .append("g")
  ...
  var bla_ = function (value, length_) {
    ...
    if (url.file == "amethi_map") {
      ...
      var _normalization = url.searchKey.district ? 75 : 18;
      // var _id = "146" //url.searchKey.check == 'no' ? g1.url.parse(location.href).searchKey.district_level : g1.url.parse(location.href).searchKey.division_level
      // var _get_rank = (1 - (Number($(".rank_"+_id).text())/_normalization) ) * 100
      var _get_rank =
        (1 -
          Number(g1.url.parse(location.href).searchKey.rank) / _normalization) *
        100;
