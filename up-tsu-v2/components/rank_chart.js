...
function get_rank_spec(st_user, _is_pdf, isDivision) {
  var conf = {
    height: st_user && _is_pdf ? 600 : st_user ? 600 : 470,
    width: 720,
    fontSize: st_user ? 7 : 10,
      ...
      },
      {
        ...
        encode: {
          update: {
            ...
            text: {
              signal:
                " (isDivision == 1) ? (((datum.area.length>7)?slice(datum.area, 0, 5)+'...':datum.area) +' (' + datum.rank + ')') : ((datum.area.length>10)?slice(datum.area, 0, 7)+'...':datum.area)  ",
            },
            fill: {
              value: "#fff",
            },
