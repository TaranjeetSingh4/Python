/* exported UI,url_update,slugify, district_name_mapping get_curr_prev_months, get_districts, render_search_bar_template,
  ...
  update_rows,common_district_template,set_labels,custom_sort_indicators,format_data, parse_url*/
/*global user_info,session_id, swal, url, indicator_mapping, JSInterface,
  render_table,render_indicator_table,data_map,ind_type:true,defaults, _def_inds, default_ind,
  load_calendar*/
// swal
...
// $('.insert_ticker_msg').template()
// $('.insert_ticker_msg_amethi').template()
    ...
    if (isIpad()) {
      ...
      if (_.includes(["table_noauth", "table"], url.file)) {
        ...
        if (url.file == "amethi_map") {
          location.href =
            "capture?ext=png&width=1400&height=2000&delay=8000&url=" +
            encodeURIComponent(location.href + "&capture=true") +
            "&start=start";
        } else if (url.file == "amethi_table") {
          location.href =
            "capture?ext=png&width=1400&height=1900&delay=8000&url=" +
            encodeURIComponent(location.href + "&capture=true") +
            "&start=start";
        } else {
          location.href =
...
//   $("[file_attr='"+$(this).attr('file_attr')+"']").remove()
// })
// .on('click', '#spec_pdf_download', function () {
//   // get_list_of_files()
// })
// .on('click', '#spec_download', function () {
//   $.ajax({
//     url: 'data/data.pdf',
//     type: 'HEAD',
//     error: function () {
//       swal("", "No file to download", "info")
//     },
//     success: function () {
//       //file exists
//       location.href = "spec_download"
//     }
//   })
// })
...
function get_districts() {
  ...
  if (url.file === "") {
    return UI.fetch_data("districts_all" + authtype, "");
  } else {
    if (url.file !== "amethi_table" && url.file !== "amethi_map") {
      let _list;
      if (_.includes([undefined, "", "district"], url.searchKey.level))
        _list = UI.fetch_data("get_list" + authtype, "")["district"];
      else _list = UI.fetch_data("get_list" + authtype, "")["division"];
      return _list;
    }
  }
}
...
function render_table_dropdown() {
  ...
  var _unique_lists = {
    ...
    niti_deepdive: "get_unique_list_niti",
  ...
  if (url.file === "amethi_table") {
    ...
    list[val] = UI.sort_list(list[val]);
  }
...
function get_area_data(
  ...
  //---------------- till here --------//
  var data = rename_keys(
    UI.fetch_data(data_map[ind_type][format], params),
    keyMap
  );
