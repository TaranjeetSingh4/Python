/* globals ajaxchain_fetch,
draw_annotations, indicator_mapping, tooltip_data,
sort_table, url:true, loader_hide, load_calendar, order_ind,
show_checkbox */
//loader_show, get_latest_date
/* exported slug_, redraw, format_inr, toCamelCase, parse_url */
...
let sort_image = {
  asc: {
    dir: "desc",
    img: "img/sort-bottom.svg",
  },
  desc: {
    dir: "asc",
    img: "img/sort-top.svg",
  },
};
let options = {
    ...
    $(".view1-table")
      .on("template", function () {
        ...
        $("tbody").scroll(function () {
          ...
          var sLeft = $(".custm-table tbody").scrollLeft();
          if (sLeft > 100) {
            $("thead th:nth-child(1)").css("position", "absolute");
            $("tbody td:nth-child(1)").css("position", "absolute");
          } else {
            $("thead th:nth-child(1)").css("position", "relative");
            $("tbody td:nth-child(1)").css("position", "relative");
          }
...
$(document)
  ...
  .on("click", "img.pos-bc, #table_view_expand_btn", function (event) {
    ...
    if ($("[id=" + cont_id + "]").hasClass("d-none")) {
      ...
      $(this).attr("src", "img/icon-collapse.svg");
      ...
      $(this).attr("src", "img/icon-expand.svg");
  ...
  .on("click", ".table_sort", function (event) {
    ...
    $(".table_sort").attr("src", "img/no-sort.svg");
  ...
  .on("click", ".info-icon-hover", function () {
    $(".info-icon-hover").attr("src", "img/info_icon.svg");
    $(this).attr("src", "img/info-more.svg");
    // if($(this).attr('src') == 'img/info_icon.svg'){
    //   $(this).attr('src', 'img/info-more.svg')
    // }else{
    //   $(this).attr('src', 'img/info_icon.svg')
    // }
  })
  ...
  //   let append_var = '&' + url.search
  //   let data = "district_download_cmo_" + view + "?_format=xlsx&_download=District_data_" + filename + ".xlsx" + append_var
  //   location.href = data
  // })
...
$.getJSON("get_maximum_date", function (data) {
  ...
  let label = moment(data[0][0]).format("DD MMM YYYY");
