/* global map, get_rank_spec, get_barchart_spec, spec1, vega, UI, url_update,render_map, indicator_mapping, user_data, load_pa_calendar, Map */
    ...
    .one("template", function () {
      ...
      if (isStateUser) {
        if (state_table_area === "district") {
          ...
          $(".level-division").removeClass("font-weight-bold");
          $(".level-district").addClass("font-weight-bold");
        } else if (state_table_area === "division") {
          $("#level").attr("checked", false);
          $(".level-district").removeClass("font-weight-bold");
          $(".level-division").addClass("font-weight-bold");
        }
      }
    })
    .template({
...
function draw_rank_chart() {
  ...
  rank_array = _.map(rank_array, function (d) {
    d.area = d.area.replace(" Division", "");
    return d;
  });
...
function get_rank_data() {
  ...
  // division or state user
    ...
    col = "composite_index";
    if (isStateUser) {
      rank_data = UI.fetch_data("division_view", rank_params);
    } else {
      rank_data = UI.fetch_data("district_view", rank_params);
    }
    rank_data = rank_data["data"];
    // _.each(rank_data, function(d){
    //   d['date'] = moment(d.date).format('YYYY-MM-DD')
...
function get_table_data(local_table_area) {
  ...
  // let fetch_url = (isStateUser || isDivision) ? 'context_data_division' : 'block_data_exe'
...
function draw_page_1() {
  ...
  // var fetch_url = (isDivision) ? 'division_data' : 'district_data_exe'
  ...
  if (isStateUser) {
    ...
    // params['division_id_num'] = district_id
    ...
    // context_data1 = UI.fetch_data('district_data_exe', params)
  }
  ...
  // final_end_array = _.sortBy(final_end_array, 'score')
  ...
  $(template_name)
    ...
    */
      ...
      if (_is_pdf) {
        $("#mapid").find("svg").css("padding-bottom", "50px");
...
$(document).on("change", "#level", function () {
  ...
  draw_table();
  // capture_function(".download_cap", (user_data[_main_area]) + "_" + moment(date).format('MMM_YYYY'))
});
