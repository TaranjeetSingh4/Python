/*global config, vega, vegam, g1, vegaTooltip, vl */
...
function vegamDraw(s, on) {
  ...
  s.spec.height = on == ".modal-body" ? 600 : 250;
  ...
  view.addEventListener("click", function (event, item) {
    ...
    if (
      ...
      Object.prototype.hasOwnProperty.call(item, "datum")
    ) {
...
function drawFilters(params) {
  ...
  $.each(config_filter, function (k, spec) {
    if (spec.type == "select") {
      $.getJSON(spec.url).done(function (data) {
